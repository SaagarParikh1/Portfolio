...
const Contact = () => {
  ...
  return (
    <section id="contact" className="py-16 bg-indigo-900 relative overflow-hidden">
      {/* Background Elements */}
      ...
      <div className="max-w-6xl mx-auto px-4 sm:px-6 lg:px-8 relative z-10">
        ...
        <div className="grid lg:grid-cols-2 gap-8">
          ...
          <div className="space-y-4">
            {/* Main Contact Card */}
            <div className="bg-gradient-to-br from-white/15 to-white/5 backdrop-blur-sm rounded-lg p-4 border border-white/20 shadow-2xl">
        ...
        <div className="bg-gradient-to-r from-purple-600 to-indigo-600 rounded-lg p-4 text-white shadow-2xl border border-purple-400/30">
              ...
              <div className="space-y-2 mb-4">
                {contactInfo.map((item, index) => (
                  <div key={index} className={`${item.bgColor} backdrop-blur-sm rounded-lg p-2 border ${item.borderColor} group hover:scale-105 transition-all duration-300`}>
                    <div className="flex items-center space-x-4">
                      <div className={`p-1 rounded-full bg-white/10 ${item.color} group-hover:scale-110 transition-transform duration-300`}>
                        {item.icon}
                      </div>
                      <div className="flex-1">
                        <h4 className="font-semibold text-white text-xs mb-1">{item.title}</h4>
                        {item.href ? (
                          <a
                            href={item.href}
                            className={`${item.color} hover:underline transition-colors duration-300 font-medium text-xs`}
                          >
                            {item.value}
                          </a>
                        ) : (
                          <p className="text-gray-200 font-medium text-xs">{item.value}</p>
                        )}
                      </div>
                    </div>
                  </div>
                ...
                <div className="flex space-x-2">
                  {socialLinks.map((link, index) => (
                    <a
                      ...
                      className={`${link.bgColor} backdrop-blur-sm p-2 rounded-lg border ${link.borderColor} transition-all duration-300 hover:scale-110 ${link.color} group`}
                    >
          ...
          <div className="bg-gradient-to-br from-white/15 to-white/5 backdrop-blur-sm rounded-lg p-4 border border-white/20 shadow-2xl">
            ...
            {submitStatus === 'success' && (
              <div className="mb-3 p-2 bg-green-600/20 border border-green-400/30 rounded-lg flex items-center space-x-3 backdrop-blur-sm">
            ...
            {submitStatus === 'error' && (
              <div className="mb-3 p-2 bg-red-600/20 border border-red-400/30 rounded-lg flex items-center space-x-3 backdrop-blur-sm">
            ...
            <form onSubmit={handleSubmit} className="space-y-3">
              <div className="grid md:grid-cols-2 gap-3">
                <div>
                  ...
                  <input
                    ...
                    className="w-full px-2 py-1 bg-white/10 border border-white/20 rounded-lg focus:ring-2 focus:ring-purple-400 focus:border-transparent transition-all duration-300 text-white placeholder-gray-300 backdrop-blur-sm text-xs"
              ...
              <div>
                ...
                <textarea
                  ...
                  onChange={handleChange}
                  rows={4}
                  className="w-full px-2 py-1 bg-white/10 border border-white/20 rounded-lg focus:ring-2 focus:ring-purple-400 focus:border-transparent transition-all duration-300 resize-none text-white placeholder-gray-300 backdrop-blur-sm text-xs"
              ...
              <button
                ...
                className="w-full bg-gradient-to-r from-purple-600 to-indigo-600 text-white px-3 py-2 rounded-lg hover:from-purple-700 hover:to-indigo-700 disabled:opacity-50 disabled:cursor-not-allowed transition-all duration-300 flex items-center justify-center space-x-2 transform hover:scale-105 shadow-lg hover:shadow-xl font-medium text-xs"
              >
                {isSubmitting ? (
                  <>
